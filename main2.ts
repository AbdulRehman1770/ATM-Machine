...
let Display_Balance = (Balance: number) => {
    console.log(`Your Current Balance is: ${Balance}`);
};

let Deposit = async (CurrentBalance: number): Promise<number> => {
...
let Fast_Cash = async (Balance: number): Promise<number> => {
...
let Withdraw_Money = async (Balance: number): Promise<number> => {
